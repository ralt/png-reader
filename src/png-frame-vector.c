...
void
PNG_frame_vector_append(struct PNG_frame_vector *vector,
			struct PNG_frame *frame)
{
	if (vector->size >= vector->capacity) {
		vector->capacity *= 2;
		vector->frames = realloc(vector->frames,
					 sizeof(struct PNG_frame) *
					 vector->capacity);
		if (vector->frames == NULL) {
			exit(errno);
		}
...
struct PNG_frame *PNG_frame_vector_get(struct PNG_frame_vector *vector,
				       int index)
{
...
void
PNG_frame_vector_set(struct PNG_frame_vector *vector, int index,
